...
function validateParameters( parameters, funcName ) {
    for( var p in parameters ) {
        if( parameters[p].type != undefined && typeof parameters[p].value != parameters[p].type ) {
            console.log( ["[msnodesql] Invalid ", parameters[p].name, " passed to function ", funcName, ". Type should be ", parameters[p].type, "." ].join('') );
...
function objectify(results) {
    ...
    for(var i = 0; i < results.meta.length; i++)
    {
        ...
        if (name !== '' && names[name] === undefined) {
            ...
            var extra = 0;
            var candidate = 'Column' + i;
            while (names[candidate] !== undefined) {
                candidate = 'Column' + i + '_' + extra++;
            }
            names[candidate] = i;
        }
    }
    ...
    var rows = [];
    var rowlen = results.meta.length
    if(results.rows.length <   results.meta.length)
    {
        rowlen = results.rows.length
    }
        ...
        this.query = function (query, paramsOrCallback, callback) {
            ...
            function onQueryRaw( err, results, more ) {
                if (chunky.callback) {
                    ...
                    else chunky.callback(err,results , more);
                }
            }
